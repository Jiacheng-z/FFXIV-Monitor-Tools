...
class WidgetList extends HTMLElement {
    ...
    addElement(name, element, sortKey) {
        ...
        if (typeof sortKey == 'number') {
            sortKeyFn = (id) => {
                let elem = this.shadowRoot.getElementById('child' + id);
                if (elem == null) {
                    return sortKey
                }
                // 获取内部倒计时
                let elemText = elem.firstChild.lastChild.lastChild.shadowRoot.getElementById('lefttext')
        ...
        this._sorted.sort(function (a, b) {
            return that._elements[a](a) - that._elements[b](b);
        });
